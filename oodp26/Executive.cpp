...
#include "DepAnal.h"
#include "../StrongComponent/StComponent.h"
#include "../Publisher/Codepublisher.h"
...
//----< usage message >----------------------------------------------
int getxmlpath(int argc,char* argv[])
{
	for (int i = 0; i < argc ; i++)
	{
		std::string str = argv[i];
		size_t p = str.find(".xml");
...
CodeAnalysisExecutive exec;
	try {
		...
		Rslt::write("\n     " + exec.systemTime());
		Rslt::flush();
		Repository* rpo= Repository::getInstance();;
		exec.getSourceFiles();
		exec.processSourceCode(true);
		exec.complexityAnalysis();
		exec.dispatchOptionalDisplays();
		exec.flushLogger();
		Files hFilesPaths = exec.cppHeaderFiles();
		Files cppFp = exec.cppImplemFiles();
		hFilesPaths.insert(hFilesPaths.end(), cppFp.begin(),cppFp.end());
		NoSqlDb<std::string> db;
		TypeAnal ta;
		TypeTable tt=ta.doTypeAnal(hFilesPaths);
		Dependency d(tt);
		int x = getxmlpath(argc, argv);
		if(x!=-1)
		db=d.DoDepAnalysis(tt,hFilesPaths,argv[x]);
		else
		db = d.DoDepAnalysis(tt, hFilesPaths,"resultx.xml");
		codepublisher cp(db);
		cp.convertHtml(hFilesPaths);
		exec.flushLogger();
		Rslt::write("\n");
		std::ostringstream out;
		exec.stopLogger();
